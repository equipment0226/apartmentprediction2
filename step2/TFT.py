"""
TFT(Temporal Fusion Transformer) 기반 서울시 아파트 매매가 예측 파이프라인
=========================================================================

데이터 : data/dong_level_meta_table.csv (2010-01 ~ 2025-12, 월 단위, 243개 동 × 25개 컬럼)
타겟   : reb__apt_sale_avg_price  (동(洞) 단위 패널)
시나리오 (VAR 파이프라인과 동일 구성):
  (1) 2010~2024.12 학습 → 2025.1~2025.12 예측 → 실측 vs 예측 차트
  (2) (1) 결과의 MAE/RMSE/MAPE/R2 등 검증 지표 저장
  (3) 전체 데이터 재학습 → 2026.1~2035.12 (10년) 예측 → 전 구간 타겟 차트
중간 산출물은 모두 step2/ 폴더에 csv/png 로 저장된다.

설계 차이 (vs VAR) :
  - VAR 은 광역(Seoul) 단일 다변량 시계열 1개 (T≈192) 였다.
  - TFT 는 cross-section(동) × 시간 패널을 그대로 학습 가능 → 243 series 활용으로
    표본 수가 크게 늘어나며 동별 이질성도 학습한다.
  - 정규화는 pytorch-forecasting 권장 방식인 GroupNormalizer (target / 동별) +
    EncoderNormalizer(robust) (unknown features) 채택. RobustScaler 와 동일한
    중앙값/IQR 기반이라 사용자의 "Robust" 요구 정신을 유지하면서 패널 구조에
    더 적합. policy__ 컬럼은 {-1,0,1} → {"loose","keep","tight"} categorical.
"""
...
# 경로 / 컬럼 정의
# ---------------------------------------------------------------------------
BASE_DIR = Path(__file__).resolve().parent          # .../step2
ROOT_DIR = BASE_DIR.parent                          # .../newtrial
DATA_CSV = ROOT_DIR / "data" / "dong_level_meta_table.csv"
OUT_DIR  = BASE_DIR

# ---------------------------------------------------------------------------
# 타겟 정의 (log-return refactor)
#   - TARGET_LEVEL_COL : 원본 가격 컬럼 (복원/차트용)
#   - TARGET_COL       : 모델이 실제로 예측하는 면 = 월간 로그수익률
#         target_logret = log(price_t / price_{t-1})  (동별)
#   [설계 근거]
#     NN time-series 모델(TFT 포함)은 학습 분포 평균으로 회귀하는 관성(mean-reversion)
#     이 있어 비정상(추세 있는) level 을 예측하면 장기 horizon 에서 보합 수렴.
#     log-수익률은 제1세차(근사 정상) 대상이므로 분포 수렴이 계속적 trend 소멸이 아니라
#     단일-step 예측 오차 최소화로 귀결 → 누적해서 level 을 복원해도 trend가 살아남.
TARGET_LEVEL_COL = "reb__apt_sale_avg_price"
...
RECENCY_HALFLIFE_MONTHS = 120   # 최근 윈도우 손실 가중치 반감기(개월).
...
def scenario_backtest(panel: pd.DataFrame) -> None:
    ...
    # p10/p90 동일 적용 (차원 간 독립 근사 — 시각화/구간용에 충분).
    # 이 근한재에서는 첫 step이 자연스럽게 마지막 실측에서 출발하므로
    # 별도의 boundary anchor 불필요 (제거됨).
    # ------------------------------------------------------------------
